package dda.apress.client;

import dda.apress.domain.ToDo;
import dda.apress.error.ToDoErrorHandler;
...
public class ToDoRestClient {
    ...
    public Iterable<ToDo> findAll() throws URISyntaxException {
        RequestEntity<Iterable<ToDo>> requestEntity =
                new RequestEntity<Iterable<ToDo>>(
                        HttpMethod.GET, new URI(properties.getUrl() + properties.getBasePath()));
        ResponseEntity<Iterable<ToDo>> responseEntity = restTemplate.exchange(
                requestEntity,
    ...
    public ToDo setCompleted(String id) {
        ...
        restTemplate.postForObject(
                properties.getUrl() + properties.getBasePath() + "/{id}?_method="+ HttpMethod.PATCH.name(),
                null,
